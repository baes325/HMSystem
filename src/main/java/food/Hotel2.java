...
public class Hotel2 extends javax.swing.JFrame {
    ...
    public Hotel2(Hotel1 mainFrame) {
    initComponents(); 
    this.model = mainFrame.getTableModel();
    
        if (this.model == null) { // 모델 초기화 확인
            JOptionPane.showMessageDialog(this, "테이블 모델을 가져오지 못했습니다. 프로그램을 다시 실행해주세요.", "오류", JOptionPane.ERROR_MESSAGE);
            dispose(); // 창 닫기
        }
    }
    ...
    try {
            ...
            model.addRow(new Object[]{menuNumber, menuName, price});
            saveToFile();
